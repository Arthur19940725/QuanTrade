...
def check_devices():
    ...
    try:
        ...
        return jsonify({
            ...
            'cpu_available': True,
            'current_device': 'cuda' if cuda_available else ('mps' if mps_available else 'cpu')
        })
    except Exception as e:
        return jsonify({
...
def search_stocks():
    ...
    try:
        if not STOCK_PREDICTOR_AVAILABLE:
            return jsonify({'error': '股票预测器不可用'}), 400
        ...
        results = stock_predictor.search_stocks(query)
...
def predict_stock():
    ...
    try:
        ...
        pred_days = int(data.get('pred_days', 7))
        lookback_days = int(data.get('lookback_days', 60))
        
        if not symbol or not market:
            return jsonify({'error': '股票代码和市场类型不能为空'}), 400
        
        if pred_days < 1 or pred_days > 30:
            return jsonify({'error': '预测天数必须在1-30天之间'}), 400
        
        if lookback_days < 30 or lookback_days > 365:
            return jsonify({'error': '历史数据天数必须在30-365天之间'}), 400
        
        # 如果有Kronos模型已加载，同步到股票预测器
        if MODEL_AVAILABLE and predictor is not None:
            # 同步模型组件到股票预测器
            stock_predictor.model = predictor.model
            stock_predictor.tokenizer = predictor.tokenizer
            stock_predictor.predictor = predictor
            stock_predictor.model_loaded = True
            print(f"✅ Kronos模型已同步到股票预测器")
        
        # 执行预测
        result = stock_predictor.predict_stock(
            symbol=symbol,
            market=market,
            pred_days=pred_days,
    ...
    try:
        markets = {
            'cn_stocks': {
                'name': 'A股',
                'description': '中国A股市场',
                'examples': ['600519', '000858', '000001']
            },
            'hk_stocks': {
                'name': '港股',
                'description': '香港股票市场',
                'examples': ['0700', '0941', '1299']
            },
            'us_stocks': {
                'name': '美股',
                'description': '美国股票市场',
        ...
        return jsonify({
            'success': True,
            'markets': markets,
            'predictor_available': STOCK_PREDICTOR_AVAILABLE
        })
        
    except Exception as e:
        return jsonify({'error': f'获取市场信息失败: {str(e)}'}), 500

@app.route('/api/real-time-price', methods=['POST'])
def get_real_time_price():
    ...
    try:
        ...
        # 获取实时价格
        real_time_price = stock_predictor.data_fetcher.get_real_time_price(symbol, market)
        ...
        if real_time_price is not None:
            return jsonify({
                ...
                'symbol': symbol,
                'market': market,
                'price': round(real_time_price, 4),
                'timestamp': datetime.datetime.now().isoformat()
            })
        else:
            return jsonify({'error': '无法获取实时价格'}), 400
        
    except Exception as e:
        return jsonify({'error': f'获取实时价格失败: {str(e)}'}), 500
